...
8. Copy the commands to the previously prepared test class.
      - Replace what we do differently:
        ...
        - Replace `SendKeys()` calls with our `ClickAndFillInWithRetries()` (like there was `driver.FindElement(By.Id("my-id")).SendKeys("my text")` then replace it with `await context.ClickAndFillInWithRetriesAsync(By.Id("my-id"), "my text")`. If there is a `Clear()` or `Click()` call before a `SendText()` call then remove it because `ClickAndFillInWithRetries()` already does these, together with retries if it doesn't succeed.
